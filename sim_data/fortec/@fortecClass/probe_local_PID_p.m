...
% data is extended to r=rho+d
%
function [poid2, aw, bw, cw]=probe_local_PID_p(obj,R0,Z0,PHI0)
%
%obj:theta_si(num2),phi_si(num3),radius_p(num2,num3,num1_p),Rax(num3),zax(num3)
%
[num2 num3 num1]=size(obj.radius_p);
...
for c=1:2
    ...
    for b=1:2
        rdata=squeeze(obj.radius_p(bp(b,c),cp(c),:));
        ...
        elseif (r0(c) >= rdata(end) && r0(c) < rdata(end)+(rdata(end)-rdata(end-1))*4.0)
            rp(b,c,1)=num1;
            rp(b,c,2)=num1;
            aw(b,c)=1.0;
            poid2(b,c,1)=bp(b,c)+num2*(cp(c)-1)+num2*num3*(rp(b,c,1)-1);
