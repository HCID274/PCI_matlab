...
function fread_EQcod3(obj,f_EQcod)
    ...
    %IRMAX=128;
    if obj.FVER == 5
        IRMAX=obj.nx0+obj.inside+obj.outside;
        ITGMAX=obj.LYM2/(obj.KZMt+1);
    else
        IRMAX=obj.IRMAX;
        ITGMAX=128;
    end
    ...
    GRCt2(s1:s2,:)=ones(s2-s1+1,1)*GRCt(a,:).*w1 ...
        +ones(s2-s1+1,1)*GRCt(a+1,:).*w2;
    GZCt2(s1:s2,:)=ones(s2-s1+1,1)*GZCt(a,:).*w1 ...
        +ones(s2-s1+1,1)*GZCt(a+1,:).*w2;
    GFCt2(s1:s2,:)=ones(s2-s1+1,1)*GFCt(a,:).*w1 ...
        +ones(s2-s1+1,1)*GFCt(a+1,:).*w2;
    end
    GRCt2(1,:)=GRCt(1,:);
    GRCt2(end,:)=GRCt(end,:);
    GZCt2(1,:)=GZCt(1,:);
    ...
    GTC_c_t(:,end)=2.0*pi;
    for b=2:IRMAX+1 %mod 2024/7/30 by Kasuya
    %if GTC_c_t(b,2)-GTC_c_t(b,1) < 0.0
    %GTC_c_t(b,1)=GTC_c_t(b,1)-2.0*pi;
    %[b 1]
    %end
    ev_r1=(GTC_c_t(b,2:end-1)-GTC_c_t(b,1:end-2)).*(GTC_c_t(b,3:end)-GTC_c_t(b,2:end-1));
    ev_r2=find(ev_r1<0.0);
    if isempty(ev_r2)==false
    GTC_c_t(b,1:ev_r2(1))=GTC_c_t(b,1:ev_r2(1))-2.0*pi;
    %[b ev_r2]
    end
    end
    GTC_c_t(1,:)=GTC_c_t(2,:);%at the axis 2024/8/1 by Kasuya
    ...
    GRC(b,s1:s2)=GRCt2(b,a)*ones(1,s2-s1+1).*w1 ...
        +GRCt2(b,a+1)*ones(1,s2-s1+1).*w2;
    GZC(b,s1:s2)=GZCt2(b,a)*ones(1,s2-s1+1).*w1 ...
        +GZCt2(b,a+1)*ones(1,s2-s1+1).*w2;
    GFC(b,s1:s2)=GFCt2(b,a)*ones(1,s2-s1+1).*w1 ...
        +GFCt2(b,a+1)*ones(1,s2-s1+1).*w2;
    GTC_f(b,s1:s2)=GTC_f_t(b,a)*ones(1,s2-s1+1).*w1 ...
        +GTC_f_t(b,a+1)*ones(1,s2-s1+1).*w2;
    %[s1 s2]
    end
    end
    GRC(:,1)=GRCt2(:,1);
