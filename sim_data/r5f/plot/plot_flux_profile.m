...
J(2:end-1,:,:)=p2(1:end-2,:,:,2)./repmat(dr2(1:end-2),1,KYM,LZM)...
      ...
      -p2(2:end-1,:,:,2).*repmat((m.*m).',IRMAX-1,1,LZM)./repmat(R2(2:end-1),1,KYM,LZM)...
...
datac2(2:end,:,:)=(p2(2:end,:,:,4))...
    .*repmat(i*m.',IRMAX,1,LZM)./repmat(obj.R(2:end),1,KYM,LZM).*conj(p2(2:end,:,:,1));
datac2(:,:,2:end)=2.0*datac2(:,:,2:end);

%vB
datac3=zeros(IRMAX+1,KYM,LZM);
datac3(2:end,:,:)=(p2(2:end,:,:,2))...
    .*repmat(i*m.',IRMAX,1,LZM)./repmat(obj.R(2:end),1,KYM,LZM).*conj(p2(2:end,:,:,3));
datac3(:,:,2:end)=2.0*datac3(:,:,2:end);

%JB
datac4_d=zeros(IRMAX+1,KYM,LZM);
datac4_d(2:end-1,:,:)=-J(2:end-1,:,:)...
    .*repmat(i*m.',IRMAX-1,1,LZM)./repmat(obj.R(2:end-1),1,KYM,LZM).*conj(p2(2:end-1,:,:,2));
