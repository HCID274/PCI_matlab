function frequency(sim,data_n)
    ...
    loglog(ky, gamma, 'LineWidth', 1.5);
    xlabel('ky');
    ylabel('\gamma');
    title('ky vs \gamma');
    figure1_path = sprintf('%s%s%s%d%s',dataC.indir,'result_PCI/figure/','GrowthRate_',data_n,'.fig')
    saveas(figure(1),figure1_path);

    figure(2)
    %errorbar(ky, omega, o_err, 'o-', 'LineWidth', 1.5);
    semilogx(ky, omega, 'LineWidth', 1.5);
    xlabel('ky');
    ylabel('\omega');
    title('ky vs \omega');
    figure1_path = sprintf('%s%s%s%d%s',dataC.indir,'result_PCI/figure/','Frequency_',data_n,'.fig')
    saveas(figure(1),figure1_path);
